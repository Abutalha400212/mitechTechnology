  ...
  return (
    <>
      <Modal show={show} className='text-info modal-info'>
        <ModalHeader><Image width='100%' src={image}/></ModalHeader>
        <Modal.Header>
          <Modal.Title>{title}</Modal.Title>
        </Modal.Header>
        <Modal.Body>Woohoo, you're in to become a premium user!</Modal.Body>
        <div className="fs-5 text-center" >Purchase Fee <span className="fs-4 fw-bold text-danger">{proFee}Tk</span></div>
        <div className="text-center">
            <h5>Payment Method</h5>
            <div className="d-flex justify-content-center align-items-center pointer">
                <Image   className="" roundedCircle width={50} src={img1}/><Image roundedCircle width={90} src={img2}/><Image roundedCircle width={90} src={img3}/>
            </div>
        </div>
        <Modal.Footer>
         <Link to='/discover'> <Button  variant="secondary" onClick={handleClose}>
            Close
          </Button></Link>
         <Link to='/discover'> <Button  variant="primary" onClick={handlePremium}>
